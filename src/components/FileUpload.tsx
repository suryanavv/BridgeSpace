...
interface FileUploadProps {
  ...
  showPasteInfo?: boolean;
}

const FileUpload = forwardRef<{ uploadFromPaste: (files: File[]) => void }, FileUploadProps>(
  ({ networkConnected, onFilesUploaded, privateSpaceKey, showPasteInfo }, ref) => {
    ...
    // Expose uploadFromPaste to parent
    useImperativeHandle(ref, () => ({
      uploadFromPaste: (files: File[]) => {
        handleUpload(files);
      },
    }));
    ...
    return (
      <div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-xl animate-fade-in">
        ...
        {showPasteInfo && (
          <div className="mb-2 text-xs text-blue-700 bg-blue-50 dark:bg-blue-900/30 rounded px-2 py-1 text-center">
            You can directly <b>paste (Ctrl+V)</b> files, images, or PDFs when the Files tab is active.
          </div>
        )}
